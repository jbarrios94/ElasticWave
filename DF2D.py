...
isx = round(nx/2) + so//2                                  # source location
f0  = 0.015 
xmax  = 1000.
zmax  = 1000.                                          # in m 
tmax  = 600                                     # simulation time in s
...
z  = (np.arange(nz)*dz) 
isz = np.where(z == 20)[0][0] + so//2

# nlayers = 5
# cp_top = 1.5
# cp_bottom = 3.5
c = np.ones((nx + so, nz + so)) * 1.5
# c[:] = cp_top  # Top velocity (background)
# cp_i = np.linspace(cp_top, cp_bottom, nlayers)
# for i in range(1, nlayers):
#       c[i*int(nz / nlayers) + 2:,:] = cp_i[i]  # Bottom velocity
...
dt = eps * np.minimum(dx, dz)/(np.max(c))              # calculate time step from stability criterion(s)
...
v = np.zeros((nx + so, nz + so))                                       # velocity

u = np.zeros((nx + so, nz + so))                                       # velocity

s = np.zeros((nx + so, nz + so))                                       # stress
...
for it in range (nt):
    #----------------------------------Df2-----------------------------------
#     du = (-u[1:-2, 1:-2] + u[2:-1, 1:-2] )/dx
#     dv = (-v[1:-2, 1:-2] + v[1:-2, 2:-1] )/dz
#     s[2:-1, 2:-1] = s[2:-1, 2:-1] - c[2:-1, 2:-1]**2 * rho[2:-1, 2:-1] * dt * (du + dv)
#     print(c[2:-1, 2:-1].shape)
#     dsx = (-s[1:-2, 2:-1] + s[2:-1, 2:-1])/dx
#     dsy = (-s[2:-1, 1:-2] + s[2:-1, 2:-1] )/dz
...
#     s[:, :so-1] = -s[:, so:2*so - 1][:, ::-1]
    ...
    s[7:, 7:] = s[7:, 7:] - c[7:, 7:]**2 * rho[7:, 7:]* dt * (du + dv)

    #s[:, so+1] = 0
    #s[:, :so-2] = -s[:, so:2*so - 2][:, ::-1]
...
fin = time.time()
print(fin - inicio)
plt.imshow(s[4:-4, 4:-4].T, cmap = 'seismic', extent = [0, xmax, zmax, 0])
print(np.max(s.T))
plt.show()